...
description: Paste line, comma, or tab-separated items, auto-detect the separator, and convert them into CSV, a Markdown table, a JSON array, or a numbered list with blank, space, and duplicate cleanup.
lang: en
...
faq:
  - q: What input format works best?
    a: One item per line works well, but comma-separated and tab-separated lists also work. Auto-detect checks tabs, line breaks, then commas to choose a separator.
  ...
  - q: Are quoted comma lists supported?
    a: Yes. A value like `"A, B", C` keeps `A, B` as one item instead of splitting inside the quotes.
---

## Why this tool was selected for today's quality pass
Recent 21:00 quality passes focused on other tools such as `volumetric-weight-calculator`, `recycling-sorting-checker`, `movie-seat-choice-simulator`, `fuel-economy-calculator`, and `image-resizer`, so this pass avoids repeating the same tool.
`list-format-converter` is a converter-style text utility where pasted messenger, spreadsheet, and README lists often include line breaks, tabs, commas, quotes, blank values, and duplicates. That made it a strong candidate for validation, parsing, and mobile input-flow improvements.
...
This tool turns one-item-per-line lists, comma-separated lists, and tab-separated lists directly into **CSV, Markdown tables, JSON arrays, and numbered lists**. Auto-detect and quote-aware comma parsing make pasted spreadsheet lists and chat snippets such as `A, "B, C", D` easier to clean safely.

## How to use it
1. Paste the list you want to convert, with one item per line or items separated by commas or tabs.
2. Keep separator auto-detect on, or choose line break, comma, or tab manually, then select CSV, Markdown table, JSON array, or numbered list.
...
### 5) Cleaning tab-separated spreadsheet values
When you copy a spreadsheet column or row, tabs and line breaks can appear together. Auto-detect helps choose the practical separator before conversion.
