---
layout: tool
title: JSON結合 | 配列・オブジェクトファイルのマージ、重複削除
description: 複数のJSONファイルをブラウザ内で安全に結合できます。配列連結、オブジェクトキー・共通配列の統合、重複削除、競合確認、結果コピー、merged.jsonの保存に対応します。
lang: ja
...
APIレスポンスをページごとに保存した場合や、複数のバッチ結果を1つの`merged.json`にまとめたい場合、外部サーバーへ再アップロードせずローカルブラウザで確認できます。
