---
layout: tool
title: リンク抽出整理ツール | URL抽出・UTM削除・CSVリンク一覧変換
description: テキスト内の http、https、www、Markdownリンクをブラウザ内で抽出し、重複削除、UTM・クリック追跡パラメータ削除、ページ内アンカー削除、ドメイン別整理、Markdown・HTML・CSV出力まで行います。
lang: ja
...
tool_type: utility
topic_cluster: text
keywords: [リンク抽出, URL整理, UTM削除, リンク重複削除, ドメイン別整理, Markdownリンク一覧, CSVリンク一覧]
related_tools: [privacy-exposure-checker, text-line-break-cleaner, filename-sanitizer]
faq:
  - q: どんなリンクを抽出できますか?
    a: http:// と https:// で始まるURL、www. リンク、Markdownリンク内のURLを抽出します。Markdownリンクの文字列は結果タイトルとして残し、文末のカンマ、ピリオド、余分な閉じ括弧なども整理します。
  - q: どの追跡パラメータを削除しますか?
  ...
  - q: 結果を文書に貼りやすい形式にできますか?
    a: はい。URLのみの一覧、Markdownの箇条書き、HTMLリンク一覧、CSVとしてコピーできます。
...
このツールは本文からURLだけを素早く抽出し、重複を減らし、よくある追跡パラメータを外して、共有しやすい一覧に整える utility 系ツールです。URLのみ、Markdown箇条書き、HTMLリンク一覧、CSVの形式でコピーでき、入力内容はサーバーへ送らず、ブラウザ内だけで処理します。

## 今日の改善対象に選んだ理由
最近の21:00品質改善は `average-speed-calculator`、`cafe-work-seat-simulator`、`blog-banned-word-checker`、`image-upscaler`、`case-converter` などに集中していたため、同じツールの繰り返しを避けました。
`link-list-cleaner` は、貼り付けた文章にMarkdownリンク、閉じ括弧、UTM・広告パラメータ、ページ内アンカー、長いURLが混ざりやすく、パーサー防御・モバイルでの確認・コピー出力を改善する価値が高い utility 型ツールとして選びました。

## 使い方
1. リンクが混ざったテキストをそのまま貼り付けます。
2. 重複削除、追跡パラメータ削除、ドメイン別整理、ページ内アンカー削除のオプションを必要に応じてオンにします。
3. コピー用の出力形式をURL一覧、Markdown箇条書き、HTMLリンク一覧、CSVから選びます。
...
### 4) Markdown・HTML・CSVの参考リンク一覧を作りたい時
Markdownリンクに文字列があれば、`- [タイトル](https://...)` の形で残せます。表計算や確認作業には `title,url,domain` のCSVとしてコピーできます。
...
## FAQ
### 1行に複数のリンクがあっても使えますか?
はい。1行の中に複数のURLがあっても、順番どおりに見つけて一覧化します。`www.` で始まるリンクは共有しやすいように `https://` 形式へ整え、MarkdownリンクはタイトルとURLを一緒に残します。
...
### ページ内アンカーは削除してもよいですか?
`#comments` や `#section-2` のようなページ内位置が不要なら削除しても問題ありません。特定の見出しやコメント位置へ直接開く必要があるリンクでは、オプションをオフにしてください。

## まとめ
リンク抽出整理ツールは、**文章中のURL、wwwリンク、Markdownリンクを抽出し、重複削除・追跡パラメータ削除・ページ内アンカー削除・ドメイン別整理・文書用の出力形式変換までまとめて行える utility 型ツール**です。
