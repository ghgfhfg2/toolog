---
layout: tool
title: リスト形式変換ツール | 行・カンマ・タブ区切りをCSV・Markdown表・JSONへ変換
description: 行ごとのリストやカンマ・タブ区切りの項目を自動判定し、CSV、Markdown表、JSON配列、番号付きリストへ変換し、空行・空白・重複も整理できます。
lang: ja
...
tool_type: converter
topic_cluster: text
keywords: [リスト変換, CSV変換, Markdown表変換, JSON配列変換, カンマ区切り変換, リスト整理]
related_tools: [text-line-break-cleaner, link-list-cleaner, case-converter]
faq:
  - q: どのような形式で入力すればよいですか？
    a: 1行に1項目ずつでも、カンマ区切りやタブ区切りでも使えます。自動判定ではタブ、改行、カンマの順に区切り候補を確認します。
  - q: 表の列名を変えられますか？
    a: はい。ヘッダー入力欄に好きな列名を入れると、CSVとMarkdown表の先頭行に反映されます。
  - q: 入力したリストはサーバーに保存されますか？
    a: いいえ。変換は現在のブラウザ内だけで実行され、サーバーへ送信・保存されません。
  - q: 引用符付きのカンマ区切りも扱えますか？
    a: はい。`"A, B", C` のような入力では、引用符内の `A, B` を1項目として保持します。
---

## 今日の改善対象として選んだ理由
最近の21:00品質改善は `volumetric-weight-calculator`、`recycling-sorting-checker`、`movie-seat-choice-simulator`、`fuel-economy-calculator`、`image-resizer` など別の計算・チェック・画像ツールに集中していたため、同じツールの繰り返しを避けました。
`list-format-converter` は、メッセンジャー、スプレッドシート、READMEから貼り付けたリストに改行、タブ、カンマ、引用符、空値、重複が混ざりやすいconverter型ツールなので、入力検証、パース、モバイル操作の改善価値が高いと判断しました。
...
このツールは、1行に1項目ずつ書かれたリストだけでなく、カンマ区切りやタブ区切りのリストも**CSV、Markdown表、JSON配列、番号付きリスト**へすぐに変換します。区切りの自動判定と引用符を考慮したカンマ解析により、スプレッドシートからコピーした値や `A, "B, C", D` のようなチャット文も整理しやすくなります。

## 使い方
1. 変換したいリストを、1行に1項目ずつ、またはカンマ・タブ区切りで貼り付けます。
2. 区切りは自動判定のまま使うか、改行、カンマ、タブから手動で選び、結果形式をCSV、Markdown表、JSON配列、番号付きリストから選びます。
...
### 4) カンマで届いた候補リストを整理する時
チャットで `A, B, C` のように届いた候補をカンマで分割し、番号付きリストやJSON配列へ変換できます。

### 5) スプレッドシートからコピーしたタブ区切りを整える時
表の列や行をコピーすると、タブや改行が混ざることがあります。自動判定を使うと、変換前に実用的な区切りを選びやすくなります。
...
### とても長いリストも一度に変換できますか？
入力は最大30,000文字で、ブラウザの停止を避けるため一度に最初の1,000項目まで変換します。それ以上のリストは分けて処理してください。
