...
description: 行ごとのリストやカンマ・タブ区切りの項目を、CSV、Markdown表、JSON配列、番号付きリストへ変換し、空行・空白・重複も整理できます。
lang: ja
...
faq:
  - q: どのような形式で入力すればよいですか？
    a: 1行に1項目ずつが基本です。オプションでカンマ区切りやタブ区切りも選べ、前後の空白や空値を整理できます。
  - q: 表の列名を変えられますか？
...
このツールは、1行に1項目ずつ書かれたリストだけでなく、カンマ区切りやタブ区切りのリストも**CSV、Markdown表、JSON配列、番号付きリスト**へすぐに変換します。ブラウザ内だけで処理されるため、仕事のリスト、会議の議題、ブログ素材、チェックリストの下書きをすばやく整理したい時に便利です。

## 使い方
1. 変換したいリストを、1行に1項目ずつ、またはカンマ・タブ区切りで貼り付けます。
2. 入力の区切りと結果形式をCSV、Markdown表、JSON配列、番号付きリストから選びます。
