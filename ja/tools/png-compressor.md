...
faq:
  ...
  - q: とても大きい写真もそのまま圧縮できますか？
    a: 25MB超または24MP超の画像はブラウザメモリの負荷が大きくなる場合があります。先にリサイズしてから圧縮するほうが安定します。
...
## 今日の改善対象にした理由
直近の品質改善は `link-list-cleaner`、`average-speed-calculator`、`cafe-work-seat-simulator`、`blog-banned-word-checker`、`image-upscaler`、`case-converter`、`grocery-budget-checker` などに集中していたため、同じツールの繰り返しを避けました。
`png-compressor` は、非対応形式、大きすぎる画像、ブラウザごとのWebP/JPEG書き出し、透過画像のJPEG変換など、つまずきやすい入力フローがあります。
今回の更新では、モバイルでも結果、警告、プレビュー、ダウンロード状態が同じ流れで分かるようにしました。
...
- 未選択実行、非対応形式、25MB超または24MP超の画像、元より大きい結果をメッセージで案内
- 透過PNGをJPEG保存すると白背景に合成されることを事前に案内
- 結果ファイルは元のファイル名をもとに安全なダウンロード名で保存
...
- モバイルでは、先にファイルを選び、出力形式と品質を決めてから圧縮実行を押すと、結果・警告・プレビュー・ダウンロードリンクが同じ画面の流れで更新されます。
...
### とても大きい写真もそのまま圧縮できますか？
25MB超または24MP超の画像はブラウザメモリの負荷が大きくなる場合があります。先に画像リサイズツールでピクセル数を下げてから圧縮するほうが安定します。
