---
layout: tool
title: パスワード生成ツール | 強力なランダムパスワードを作成・非表示・コピー
description: 4〜128文字のランダムパスワードをブラウザ内で生成。英大文字・小文字・数字・サイト互換記号、紛らわしい文字の除外、画面非表示、エントロピー、強度目安、個別・一括コピーに対応します。
lang: ja
...
faq:
  ...
  - q: 推奨の文字数は？
    a: 通常アカウントは12文字以上、金融・仕事・管理者アカウントは16文字以上が推奨です。特に重要なアカウントでは24文字のようにさらに長めにすると安心です。
  - q: サイトで記号が使えない場合は？
    a: まずサイトで使いやすい記号セットを試してください。それでも拒否される場合は記号を外し、長さを16文字以上に増やすのが実用的です。
...
## 主な機能
- **長さ（4〜128）** と **生成数（1〜20）** を設定、12・16・24文字の推奨ボタン
- 英大文字・英小文字・数字・記号の組み合わせ選択
- サイトで使いやすい記号セット/広めの記号セットを選択
- 紛らわしい文字の除外（`O/0`, `l/I/1`, `B/8`, `S/5`, `Z/2` など）
- 強度目安・文字プール・組み合わせ数・エントロピーを表示
- 一括生成時の重複最小化、個別コピーと一括コピー
- 共有画面や外出先で使うときに生成結果を画面上でマスク
- 長さ・生成数・文字種選択のエラーをすぐに表示

## 使い方
1. パスワードの長さと生成数を入力
2. 含める文字種と記号セットを選択
3. 必要なら紛らわしい文字の除外や画面表示オプションを調整
4. 生成ボタンを押し、必要な候補だけ個別コピーまたは一括コピー
...
## FAQ
### 生成されたパスワードは安全ですか？
ブラウザの暗号学的乱数API（`crypto.getRandomValues`）を使って生成します。

### 画面上でパスワードを隠せますか？
はい。`生成結果を画面に表示`をオフにすると、テキストエリアと候補一覧がマスクされます。コピー操作はそのまま使えます。

### 記号なしで生成してもよいですか？
可能です。ただし文字プールが減るため、推測されにくさは下がる場合があります。

### 強度目安は絶対的なものですか？
いいえ。長さと文字プールに基づく参考値です。実際の安全性は、パスワードの使い回しや2段階認証（2FA）の有無にも大きく左右されます。

### 一括生成で重複することはありますか？
可能な範囲では重複しない候補を優先します。ただし文字プールが小さい、または長さが短い場合は組み合わせ数が限られることがあります。

## まとめ
パスワード生成ツールは、アカウントを守るための基本ツールです。
**長さ・文字種・読みやすさのオプション** を調整して、実用しやすく安全なパスワードをすばやく作成できます。
