(() => {
  ...
  if (slug === 'png-compressor') {
    ...
    const clear = document.getElementById('pc-clear');
    ...
    const pcText = {
      ko: {
        ...
        tooLarge: '25MB 초과 또는 24MP 초과 이미지는 브라우저 압축이 불안정할 수 있습니다. 먼저 이미지 리사이저로 크기를 줄여 주세요.',
        ...
        jpegAlpha: '투명 배경이 있는 PNG/WebP를 JPEG로 저장하면 투명 영역은 흰색 배경으로 합성됩니다.',
        larger: '결과 용량이 원본보다 커졌습니다. 이미 최적화된 이미지일 수 있으니 WebP/JPEG 품질을 낮추거나 원본을 그대로 쓰는 편이 나을 수 있습니다.',
        cleared: '선택한 이미지와 압축 결과를 초기화했습니다.'
      },
      en: {
        ...
        tooLarge: 'Images over 25 MB or 24 MP can be unreliable in the browser. Resize the image first.',
        ...
        jpegAlpha: 'Transparent PNG/WebP pixels are flattened onto a white background when saving as JPEG.',
        ...
        cleared: 'Cleared the selected image and compression result.'
      },
      ja: {
        ...
        tooLarge: '25MB超または24MP超の画像はブラウザ内圧縮が不安定になる可能性があります。先にリサイズしてください。',
    ...
    const safeBaseName = (name = 'compressed') => {
      const withoutExt = String(name).replace(/\.[^.]+$/, '') || 'compressed';
      return withoutExt
        .normalize('NFKD')
        .replace(/[^\w.-]+/g, '-')
        .replace(/^-+|-+$/g, '')
    ...
    const resetTool = (message = pcText.empty, state = '') => {
      setDownloadReady(false);
      setWarning('');
      clearCanvas();
      img = null;
    ...
    file?.addEventListener('change', () => {
      ...
      i.onload = () => {
        ...
        if ((format?.value || '') === 'image/jpeg' && originType !== 'image/jpeg') {
    ...
    format?.addEventListener('change', () => {
      ...
      if (format.value === 'image/png') {
        ...
        setWarning('');
      } else if (format.value === 'image/jpeg' && originType && originType !== 'image/jpeg') {
        setWarning(pcText.jpegAlpha);
    ...
    run?.addEventListener('click', async () => {
      ...
      link.download = `${safeBaseName(file.files?.[0]?.name)}-compressed.${extByMime(mime)}`;
      ...
      else if (mime === 'image/jpeg' && originType !== 'image/jpeg') setWarning(pcText.jpegAlpha, 'warning');
    });
  }
  ...
  if (slug === 'filename-sanitizer') {
    ...
    const t = fsText[pageLang] || fsText.ko;
    ...
    const analyze = () => {
      ...
      if (numberEnabledEl.checked && (!Number.isInteger(rawStart) || rawStart < 1 || rawStart > 9999)) {
        ...
        resetStats();
        inputCountEl.textContent = allLines.length.toLocaleString(numberLocale);
        setSummary(t.invalidStart, 'error');
        return;
      }
      ...
      const outputs = lines.map((line, index) => {
        ...
        if (numberEnabledEl.checked) parts.push(pad(start + index, numberWidth));
        parts.push(name);
  ...
  if (slug === 'json-merge') {
    ...
    const manualMeter = document.getElementById('jm-manual-meter');
    ...
    const renderFileList = (files) => {
      ...
      files.slice(0, 8).forEach((file) => {
        ...
        pill.className = 'jm-file-pill';
        const text = document.createElement('span');
        text.className = 'jm-file-pill__text';
        text.textContent = jmText.filePill(file.name, formatKb(file.size));
        const remove = document.createElement('button');
        remove.className = 'jm-file-pill__remove';
        remove.type = 'button';
        remove.textContent = 'x';
        remove.setAttribute('aria-label', jmText.removeFile(file.name));
    ...
    clearBtn.addEventListener('click', () => {
      ...
      manualInput.setAttribute('aria-invalid', 'false');
      fileList.replaceChildren();
      manualMeter.textContent = jmText.manualEmpty;
      manualMeter.dataset.state = '';
